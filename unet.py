...
class myUnet(object):
	def __init__(self, img_rows = 512, img_cols = 512):
		...
		self.mydata = dataProcess(self.img_rows, self.img_cols)
# 参数初始化定义
	def load_data(self):
...
# 如果需要修改输入的格式，那么可以从以下开始修改，上面的结构部分不需要修改
	def train(self):
		...
		np.save('./images/test_results/imgs_mask_test.npy', imgs_mask_test)

	def save_img(self):
		print("array to image")
		imgs = np.load('./images/test_results/imgs_mask_test.npy')
		imgs_test_name = self.mydata.load_test_name()
		for i in range(imgs.shape[0]):
			img = imgs[i]
			img = array_to_img(img)
			name = imgs_test_name[i]
			img.save("./images/test_results/%s.jpg"%(name))
